...
class Database:
    ...
    async def add_user_CourseRegistrations(self, name, phonenumber, id):
        sql = "INSERT INTO CourseRegistrations (name, phonenumber, id) VALUES($1, $2, $3) returning *"
        return await self.execute(sql, name, phonenumber, id, fetchrow=True)

    # async def select_all_users_CourseRegistrations(self):
    #     sql = "SELECT * FROM CourseRegistrations"
    #     return await self.execute(sql, fetch=True)

    # async def select_user_CourseRegistrations(self, **kwargs):
    #     sql = "SELECT * FROM CourseRegistrations WHERE "
    #     sql, parameters = self.format_args(sql, parameters=kwargs)
    #     return await self.execute(sql, *parameters, fetchrow=True)

    # async def count_users_CourseRegistrations(self):
    #     sql = "SELECT COUNT(*) FROM CourseRegistrations"
    #     return await self.execute(sql, fetchval=True)

    # async def update_user_username_CourseRegistrations(self, username, telegram_id):
    #     sql = "UPDATE CourseRegistrations SET username=$1 WHERE id=$2"
    #     return await self.execute(sql, username, telegram_id, execute=True)

    # async def delete_users_CourseRegistrations(self):
    #     await self.execute("DELETE FROM CourseRegistrations WHERE TRUE", execute=True)

    # async def drop_users(self):
    #     await self.execute_CourseRegistrations("DROP TABLE CourseRegistrations", execute=True)
    ...
    async def get_products_count(self, category_code):
